...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const fetchYahooStockData = async (symbol: string) => {
      ...
      const baseUrls = [
        `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?region=${primaryRegion}&lang=${primaryLang}&interval=15m&range=5d`,
        `https://query2.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?region=${primaryRegion}&lang=${primaryLang}&interval=15m&range=5d`,
      ...
      for (const url of urlsToTry) {
        try {
          console.log(`[Market Data Edge] Attempting fetch from: ${url}`);
          ...
          if (response.ok) {
            ...
            if (data?.chart?.result?.[0]) {
              console.log(`[Market Data Edge] Successfully retrieved real-time price from Yahoo Finance via: ${url}`);
              return data.chart.result[0];
            }
          } else {
            console.warn(`[Market Data Edge] Bad response status ${response.status} from: ${url}`);
          }
        } catch (e) {
          console.error(`[Market Data Edge] Exception during fetch from ${url}:`, e);
        }
      }
      return null;
    };
    ...
    const lookupReferencePrice = (symbol: string): { price: number; isIndian: boolean; name: string } => {
    ...
    const getFallbackStockData = (symbol: string) => {
      const ref = lookupReferencePrice(symbol);
      const currencyPr = "₹";
    ...
    const getMacdSignal = (prices: number[]): string => {
    ...
    const calculateLevels = (prices: number[]) => {
    ...
    const generateContentWithFallback = async (ai: any, promptText: string, schema: any) => {
      const fallbackModels = ["gemini-3.1-flash-lite", "gemini-3.5-flash"];
      let lastError: any = null;

      for (const model of fallbackModels) {
        try {
          console.log(`[AI Engine Edge] Dispatching request using model: ${model} (1 attempt)`);
          ...
          console.warn(`[AI Engine Edge] API call failed on ${model} (1 attempt):`, errMsg);
        }
      }
